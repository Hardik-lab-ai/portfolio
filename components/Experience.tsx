    ...
    <section id="experience" style={{ background: "var(--bg-alt)", padding: "100px 0", position: "relative" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 32px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 72 }}>
          ...
          <div className="ruler-divider" style={{ maxWidth: 320, margin: "0 auto" }} />
        </div>
        ...
        <div style={{ position: "relative" }}>
          ...
          {TIMELINE.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Card — lighter feel, left accent bar instead of heavy top border */}
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
                    <div>
                      <div style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 15.5, color: "var(--text-1)", lineHeight: 1.2 }}>
                        {item.role}
                      </div>
                      <div style={{ color: item.color, fontSize: 12.5, fontWeight: 600, marginTop: 2 }}>{item.company}</div>
                    </div>
                    <span style={{
                  ...
                  <div style={{ display: "flex", gap: 16, marginBottom: 14, flexWrap: "wrap" }}>
                    <span style={{ color: "var(--text-3)", fontSize: 11.5 }}>{item.period}</span>
                    <span style={{ color: "var(--text-3)", fontSize: 11.5 }}>{item.location}</span>
                  </div>

                  {/* Bullets */}
                  <ul style={{ listStyle: "none", margin: 0, padding: 0, marginBottom: 14 }}>
                    {item.highlights.map((h, hi) => (
                      <li key={hi} style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 6, color: "var(--text-2)", fontSize: 12.5, lineHeight: 1.55 }}>
                        <span style={{ color: item.color, marginTop: 5, flexShrink: 0, fontSize: 5 }}>●</span>
                        {h}
                      </li>
                    ))}
                  </ul>

                  {/* Tool chips — pill shaped */}
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {item.tools.map(tool => (
                      <span key={tool} style={{
                        background: "var(--chip-bg)", border: "1px solid var(--chip-border)",
                        color: "var(--text-3)", fontSize: 10.5, padding: "3px 10px",
          ...
          <div style={{
            ...
            background: "var(--accent)", boxShadow: "0 0 12px rgba(217,107,67,0.40)",
          }} />
        </div>
      </div>
    </section>
